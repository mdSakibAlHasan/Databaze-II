...
original_row_info data_value[TOTAL];
...
void load_data_for_tree(int tree_index)
{
    shuffle(data_value, data_value + TOTAL - 1, default_random_engine(tree_index));

    srand(tree_index);
    for (int i = 0; i < MAX_COL; i++)
    {
        rand_array[tree_index][i] = rand() % (MAX_COL + 1);
    }
    ...
    for (int i = 0; i < TREE_DATA; i++)
    {
        for (int k = 0; k < COL; k++)
        {
            values[i].attribute_info[k] = data_value[i].attribute_info[rand_array[tree_index][k]];
        }
...
bool testing(node **root, row_info test_data)
{
    ...
    {
        char ch = find_decision(root[i], test_data);
...
void random_forest()
{
    ...
    node **root;
    root = new node *[NUMBER_OF_TREE];

    for (int i = 0; i < NUMBER_OF_TREE; i++)
    {
        load_data_for_tree(i);
        root[i] = create_tree();
    ...
    cout << "Accuracy is: " << probability * 100 << endl;
}
